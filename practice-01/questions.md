...
**Q7.** Return every author who has **not** written any book in this dataset.

**Q8.** List every distinct genre present in the dataset, with no duplicates.

**Q9.** Return the titles of books published by **either** Tor Books **or** Gollancz.
...
**Q13.** Return the average page count across all books that have a known page count.

**Q14.** For each genre, return the year of the most recent book in that genre.

**Q15.** Return one column called `?label` formatted like `"Kindred (1979)"` for every book that has a known year.

**Q16.** Return the books written by either Le Guin or Liu Cixin (look them up by author URI: `d:leguin`, `d:liu`).

**Q17.** Return the names of authors who wrote **more than one** book in this dataset.
...
**Q20.** `lib:Novel` is declared `rdfs:subClassOf lib:Book`. Write a query that returns every instance of `lib:Book` **including** instances declared only as `lib:Novel`.
